...
export class Jaeger extends cdk.Construct {
    constructor(scope: cdk.Construct, id: string, props: StackProps) {
        ...
        const doc = readYamlDocument(__dirname + '/assets/jaeger-bhojpur.yaml');
        const bhojpurJaeger = new KubernetesManifest(cluster.stack, "bhojpur-jaeger", {
            cluster,
            overwrite: true,
            manifest: [loadYaml(doc)],
        });
        bhojpurJaeger.node.addDependency(helmChart);
    }
}

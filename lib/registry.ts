...
export class Registry extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props: RegistryProps) {
        ...
        const BhojpurRegistryAccess = new iam.Policy(this, 'RegistryAccess', {
            policyName: 'BhojpurS3Access',
            statements: [
        ...
        storage.attachInlinePolicy(BhojpurRegistryAccess);

        const userName = `registry-storage-${props.clusterName}`.toLowerCase();
        const user = new iam.User(this, 'BhojpurIAMUserS3', {
            userName,
            groups: [storage]
        });

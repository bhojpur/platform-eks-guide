...
function install() {
    ...
    # The reason for this change is related to the number of containers we can have in AWS EC2 instances
    ...
    if ${AWS_CMD} iam get-role --role-name "${CLUSTER_NAME}-region-${AWS_REGION}-role-eksadmin" > /dev/null 2>&1; then
        ...
        echo "Creating Role for AWS EKS access"
    ...
    # Manage AWS IAM users and roles https://eksctl.io/usage/iam-identity-mappings/
    if ! eksctl get iamidentitymapping --cluster "${CLUSTER_NAME}" --arn "${KUBECTL_ROLE_ARN}" > /dev/null 2>&1; then
        echo "Creating mapping from AWS IAM role ${KUBECTL_ROLE_ARN}"
    ...
    MYSQL_BHOJPUR_PASSWORD=$(openssl rand -hex 18)
    MYSQL_BHOJPUR_SECRET="mysql-bhojpur-token"
    MYSQL_BHOJPUR_ENCRYPTION_KEY='[{"name":"general","version":1,"primary":true,"material":"7uGh3q8y2DYryJwrVMHs5kWXJlqvHWWt/KJuNi25edI="}]'
    SECRET_STORAGE="object-storage-bhojpur-token"

    # generated password cannot excede 41 characters (RDS limitation)
    SSM_KEY="/bhojpur/cluster/${CLUSTER_NAME}/region/${AWS_REGION}"
    ${AWS_CMD} ssm put-parameter \
        --overwrite \
        --name "${SSM_KEY}" \
        --type String \
        --value "${MYSQL_BHOJPUR_PASSWORD}" \
...
EOF
    ...
    kubectl create secret generic "${MYSQL_BHOJPUR_SECRET}" \
        ...
        --from-literal=username="${MYSQL_BHOJPUR_USERNAME}" \
    ...
    local CONFIG_FILE="${DIR}/platform-config.yaml"
    ...
    yq e -i ".database.external.certificate.name = \"${MYSQL_BHOJPUR_SECRET}\"" "${CONFIG_FILE}"
    ...
    yq e -i ".application.runtime.fsShiftMethod = \"shiftfs\"" "${CONFIG_FILE}"
    ...
    # Patch proxy service to remove use of cloud load balancer. In AWS EKS we use AWS ALB.
...
EOF
    ...
    until [ -n "$(kubectl get ingress bhojpur -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')" ]; do
        sleep 5
    done

    ALB_URL=$(kubectl get ingress bhojpur -o json | jq -r .status.loadBalancer.ingress[0].hostname)
    if [ -n "${ALB_URL}" ];then
        printf '\nAWS Load balancer hostname: %s\n' "${ALB_URL}"
    fi
}
...
function uninstall() {
    ...
    read -p "Are you sure you want to delete: Bhojpur.NET Platform, Services/Registry, Services/RDS, Services, Addons, Setup (y/n)? " -n 1 -r
    if [[ $REPLY =~ ^[Yy]$ ]]; then
